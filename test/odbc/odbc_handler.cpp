...
string RetrieveBbfVersion() {

  OdbcHandler con;
  SQLCHAR version[256];
  RETCODE rcode;
  string version_query = "SELECT AURORA_VERSION();";

  string connection_str = con.GetConnectionString();
  // Check if connected
  con.AllocateEnvironmentHandle();
  con.AllocateConnectionHandle();
  SQLSetConnectAttr(con.GetConnectionHandle(), SQL_LOGIN_TIMEOUT, (SQLPOINTER)10, 0); // 10 seconds
  rcode = SQLDriverConnect(con.GetConnectionHandle(), nullptr, (SQLCHAR *) connection_str.c_str(), SQL_NTS, nullptr, 0, nullptr, SQL_DRIVER_COMPLETE);

  if (!con.IsSqlSuccess(rcode)) {
    return "There was an issue connecting when retrieving the version\n";
  }
  con.AllocateStmtHandle();
  rcode = SQLExecDirect(con.GetStatementHandle(), (SQLCHAR*) version_query.c_str(), SQL_NTS);
  if (!con.IsSqlSuccess(rcode)) {
    return "There was an issue executing the query when retrieving the version. Perhaps this isn't a babelfish instance?\n";
  }

  rcode = SQLFetch(con.GetStatementHandle()); 
  if (!con.IsSqlSuccess(rcode)) {
    return "There was an issue fetching the version\n";
  }

  rcode = SQLGetData(con.GetStatementHandle(), 1, SQL_C_CHAR, &version, sizeof(version), 0);
  if (!con.IsSqlSuccess(rcode)) {
    return "There was an issue getting the version\n";
  }
